...
export class DashboardService {
  ...
  /** -------------------
   * IMPROVED: Batch Multiple Dashboards - Single Fetch
   * ------------------- */
  async getMultipleDashboards(
    dashboards: string[],
    mode: 'live' | 'historic' | 'range',
    ...
    if (cachedRawData) {
      console.log(`⚡ Batch cache hit for: ${baseCacheKey}`);
      rawData = cachedRawData;
    } else {
      // ✅ Fetch data only once for the first dashboard's projection
      const firstDashboard = dashboards[0];
      const config = this.dashboardConfigs[firstDashboard];

      if (!config) {
        throw new Error(`Dashboard ${firstDashboard} not found`);
      }

      // ✅ Use a combined projection that includes all required fields
      const combinedProjection = this.getCombinedProjection(dashboards);
      ...
      console.log('Executing single pipeline for multiple dashboards...');
      rawData = await this.collection.aggregate(pipeline).toArray();
    ...
    await Promise.all(
      dashboards.map(async (dashboard) => {
        try {
          results[dashboard] = this.processDataForDashboard(
            dashboard,
            mode,
            rawData,
          );
  ...
  private getBaseCacheKey(mode: string, start?: string, end?: string): string {
    ...
    const cleanStart = start ? start.trim() : '';
    const cleanEnd = end ? end.trim() : '';

    return `data_${mode}_${cleanStart}_${cleanEnd}`.replace(/\s+/g, '');
  }
  ...
  /** -------------------
   * FIXED: Aggregation Pipeline with Clean Parameters
   * ------------------- */
  private buildAggregationPipeline(
    mode: string,
    projection: Record<string, number>,
    ...
    console.log('Building pipeline with cleaned parameters:', {
      mode,
      cleanStart,
      cleanEnd,
    });
  ...
  clearCache(): void {
    ...
    this.cache.clear();
    console.log(`🧹 Cache cleared: ${beforeSize} entries removed`);
  }
  ...
  /** -------------------
   * NEW: Debug method to check cache keys
   * ------------------- */
  ...
  /** -------------------
   * NEW: Method to find similar cache keys
   * ------------------- */
  findSimilarCacheKeys(mode: string, start?: string, end?: string): any {
    const cleanStart = start ? start.trim() : '';
    const cleanEnd = end ? end.trim() : '';
    const searchKey = `data_${mode}_${cleanStart}_${cleanEnd}`.replace(
      /\s+/g,
      '',
    );

    const allKeys = Array.from(this.cache.keys());
    const similarKeys = allKeys.filter((key) => key.includes(mode));

    return {
      searchKey,
      similarKeys,
      exactMatch: allKeys.includes(searchKey),
    };
  }
