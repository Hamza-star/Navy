...
export class ReportsService {
  ...
  async getReport(dto: {
    ...
    // 1️⃣ Build regex query (day-level filtering)
    const query: any = {};
    if (dto.fromDate && dto.toDate) {
      if (dto.fromDate === dto.toDate) {
        query.timestamp = { $regex: `^${dto.fromDate}` };
    ...
    // 2️⃣ Projection based on towerType
    let projection: Record<string, number> | undefined;
    const sampleDoc = await this.usageModel.findOne().lean();
    if (sampleDoc) {
      const towerFields = Object.keys(sampleDoc).filter((k) =>
    ...
    // 3️⃣ Fetch all docs (matching days/projection)
    const data = await this.usageModel.find(query, projection).lean();
    ...
    // 5️⃣ Apply filtering based on fromDate/toDate and start/end times
    let startSec = dto.startTime ? hmsToSeconds(dto.startTime) : 0;
    let endSec = dto.endTime ? hmsToSeconds(dto.endTime) : 86399;
    ...
    const filteredData = data.filter((doc) => {
      ...
      let adjustedSeconds = docSeconds;
      if (crossDay && docDate === dto.toDate) adjustedSeconds += 24 * 3600;
      ...
      if (docDate === dto.toDate)
        return adjustedSeconds <= roundTo15(endSec, true);
      if (docDate > dto.fromDate && docDate < dto.toDate) return true;

      return false;
    });

    // 6️⃣ Build 15-min interval map (one doc per 15-min interval: earliest doc wins)
    const pad = (n: number) => n.toString().padStart(2, '0');
    const intervalMap = new Map<string, any>();

    for (const doc of filteredData) {
      const h = parseInt(doc.timestamp.substring(11, 13), 10);
      const rawMin = parseInt(doc.timestamp.substring(14, 16), 10);
      const m = Math.floor(rawMin / 15) * 15; // floor to interval
      let hh = h;
      let dayStr = doc.timestamp.substring(0, 10);

      // handle minute rollover (shouldn't happen with floor, but keep safe)
      if (m === 60) {
        // rarely possible if rawMin was 60 (not realistic), but safe-guard
        const nextDay = new Date(dayStr);
        nextDay.setDate(nextDay.getDate() + 1);
        dayStr = nextDay.toISOString().substring(0, 10);
        hh = (hh + 1) % 24;
      ...
      const intervalKey = `${dayStr}T${pad(hh)}:${pad(m)}`; // e.g. 2025-10-01T09:45
      const existing = intervalMap.get(intervalKey);
      if (!existing || new Date(doc.timestamp) < new Date(existing.timestamp)) {
        intervalMap.set(intervalKey, doc);
      }
    ...
    // 7️⃣ NEW: Include the FIRST DB document on toDate whose time >= endTime (user requirement)
    if (dto.endTime) {
      const safeEndTime = dto.endTime; // local non-optional copy
      const [eh] = safeEndTime.split(':').map(Number); // hour
      // find first doc on toDate with seconds >= endTime
      const candidate = data
        .filter(
          (doc) =>
            doc.timestamp.startsWith(dto.toDate) &&
            timeStrToSeconds(doc.timestamp) >= hmsToSeconds(safeEndTime),
        )
        .sort(
          (a, b) =>
            new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime(),
      ...
      if (candidate) {
        // compute the same rounded interval key used earlier for the candidate
        const ch = parseInt(candidate.timestamp.substring(11, 13), 10);
        const cMinRaw = parseInt(candidate.timestamp.substring(14, 16), 10);
        const cMin = Math.floor(cMinRaw / 15) * 15;
        let cDay = candidate.timestamp.substring(0, 10);
        let cHour = ch;
        if (cMin === 60) {
          cHour = (cHour + 1) % 24;
          const nextDay = new Date(cDay);
          nextDay.setDate(nextDay.getDate() + 1);
          cDay = nextDay.toISOString().substring(0, 10);
        }
        const candidateIntervalKey = `${cDay}T${pad(cHour)}:${pad(cMin)}`;

        // only add candidate if that interval key isn't already present
        if (!intervalMap.has(candidateIntervalKey)) {
          intervalMap.set(candidateIntervalKey, candidate);
        }
      }
      // if no candidate found, we do not add anything (per latest requirement to pick first doc >= endTime)
    }

    // 8️⃣ Sort by key (key is YYYY-MM-DDTHH:MM) to get chronological order
    const sortedData = Array.from(intervalMap.values()).sort(
      (a, b) =>
        new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime(),
    );

    // 9️⃣ Final report output (realtime / efficiency)
    const ambientAirTemp = 30;
    const wetBulbTemp = 25;
    const fanAmpere = 15;
    ...
    if (dto.reportType === 'realtime') {
      return {
        message:
          'Report (15-min intervals, includes first DB document at/after endTime hour)',
        data: sortedData.map((doc) => ({
          timestamp: doc.timestamp,
          supplyTemp: doc[`${dto.towerType}_TEMP_RTD_01_AI`] ?? null,
          returnTemp: doc[`${dto.towerType}_TEMP_RTD_02_AI`] ?? null,
    ...
    if (dto.reportType === 'efficiency') {
      ...
      const finalData = sortedData.map((doc) => {
        const hotWaterTemp = doc[hotWaterTempKey];
        const coldWaterTemp = doc[coldWaterTempKey];
        const returnWaterFlow = doc[returnWaterFlowKey];
      ...
      return {
        message:
          'Report (15-min intervals, includes first DB document at/after endTime hour — efficiency)',
        data: finalData,
      };
    }
