...
export class DashboardService {
  ...
  /** -------------------
   * Prewarm Support Helpers
   * ------------------- */
  async prewarmDashboard(
    dashboard: string,
    mode: 'live' | 'historic' | 'range' = 'range',
    start?: string,
    end?: string,
  ) {
    const key = this.getCacheKey(dashboard, mode, start, end);
    console.log(`🚀 Prewarming ${dashboard} (${mode})`);
    const data = await this.getDashboardData(dashboard, mode, start, end);
    this.setCache(key, data);
    console.log(`✅ ${dashboard} cached`);
  }

  async prewarmAllDashboards() {
    const dashboards = Object.keys(this.dashboardConfigs);

    const rangeStart = '2025-10-01T00:00:00.000Z';
    const rangeEnd = '2025-10-05T00:00:00.000Z';

    await Promise.allSettled(
      dashboards.map((dash) =>
        this.prewarmDashboard(dash, 'range', rangeStart, rangeEnd).catch(
          (err) => console.error(`⚠️ Failed prewarm for ${dash}:`, err.message),
        ),
      ),
    );

    console.log('🔥 All dashboards prewarmed in memory');
  }
}
